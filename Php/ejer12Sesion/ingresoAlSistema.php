<?php
    ...
    if (!isset($_SESSION['idSesion'])) {  //no definida
        ...
        $sql = "SELECT * FROM usuarios WHERE login = :login AND password = :password;"; // crea consulta sql

        $stmt = $dbh->prepare($sql); // preparo consulta sql

        $stmt->bindParam(':login', $login); // previene ataques de inyeccion sql
        $stmt->bindParam(':password', $hashedpass);

        $stmt->setFetchMode(PDO::FETCH_ASSOC); // modo de consulta a array asociativo, devuelve un asociativo
